// app/providers/ReconcilerProvider.tsx
"use client";
...
/** Cheap equality: compare ids-in-order (great for our flows). */
...
export function ReconcilerProvider({
  ...
  // Persist to localStorage on change (only when values actually change)
  useEffect(() => {
    try {
  ...
  const setTransactions = useCallback((next: SetTransactions) => {
    _setTransactions((prev) => {
      const value = typeof next === "function" ? (next as any)(prev) : next;
      // Always accept; categories/notes/etc. can change with same IDs
